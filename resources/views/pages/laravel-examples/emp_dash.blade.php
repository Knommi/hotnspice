<x-layout bodyClass="g-sidenav-show  bg-gray-200">
    <x-navbars.emp_sidebar activePage='dashboard'></x-navbars.emp_sidebar>

    <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg ">
        <!-- Navbar -->
        <x-navbars.navs.emp_auth titlePage="Dashboard"></x-navbars.navs.emp_auth>
        <!-- End Navbar -->
        @foreach($emp as $e)
        <div class="container">
            <h5>hello {{$e->name}}</h5>
            <form action="/add_attendance/{{$e->id}}" method="POST">
                @csrf
                    <input type="password" id="password" placeholder="enter password" name="password"><br><br>
                    @error('password')
                            <p class='text-danger inputerror'>{{ $message }} </p>
                    @enderror
                    <input type="hidden" name="eid" value="{{$e->id}}" readonly>
                    <input type="text" name="date" value="{{$date}}" readonly>
                    <input type="text" name="time" value="{{$time}}" readonly>
                    <label for="status">Choose a current status:</label>
                    <select id="status" name="status">
                        <option value="clock_in">Clock in</option>
                        <option value="lunch_in">Lunch in</option>
                        <option value="lunch_out">Lunch out</option>
                        <option value="clock_out">Clock</option>
                    </select>
                    @error('status')
                            <p class='text-danger inputerror'>{{ $message }} </p>
                    @enderror
                ...
                <div class="col-xl-3 col-sm-6 mb-xl-0 mb-4">
                    <div class="card">
                        <div class="card-header p-3 pt-2">
                            ...
                            <div class="text-end pt-1">
                                <p class="text-sm mb-0 text-capitalize">Current Time</p>
                                <br>
